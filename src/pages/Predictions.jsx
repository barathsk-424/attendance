...
export default function Predictions() {
  ...
  async function fetchData() {
    const [semRes, attRes, marksRes] = await Promise.all([
      supabase.from('semester_results').select('student_id, percentage, grade, result_status, students(full_name, roll_number)'),
      supabase.from('attendance').select('student_id, status'),
      supabase.from('semester_subject_marks').select('marks_obtained, max_marks, subjects(name)')
    ])
